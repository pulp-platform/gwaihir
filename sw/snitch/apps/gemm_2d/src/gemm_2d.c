...
static inline void allocate_l2_buffers(gemm_args_t *largs ) {
    ...
    uint32_t mem_tile_idx = gw_closest_mem_tile(snrt_cluster_idx());

    uintptr_t a_off = gw_l2_tile_offset((uintptr_t) largs -> a);
    uintptr_t c_off = gw_l2_tile_offset((uintptr_t) largs -> c);
    uintptr_t a_dst = gw_l2_tile_address(mem_tile_idx) + a_off;
    uintptr_t c_dst = gw_l2_tile_address(mem_tile_idx) + c_off;
...
static inline void write_back_c_tiles(gemm_args_t* largs, uint32_t m_tile_size,
    ...
    c_dst = gw_l2_tile_address(0) + gw_l2_tile_offset( (uintptr_t) c_src);
