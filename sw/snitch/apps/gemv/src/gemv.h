...
static inline void single_core_gemv_unrolled(uint32_t trans, uint32_t m, uint32_t n,
                                    double alpha, double *a, uint32_t lda,
                                    double *x, uint32_t incx, double *y) {
#ifdef SNRT_SUPPORTS_FREP
    ...
    for (uint32_t i = 0; i < m; i++) {
        ...
        asm volatile(
            ...
            "fadd.d %[acc0], %[acc0], %[acc1] \n"
            "fadd.d %[acc2], %[acc2], %[acc3] \n"
            "fadd.d %[acc0], %[acc0], %[acc2] \n"
            "fmul.d %[acc0], %[acc0], ft3 \n"
            : [acc0] "+f"(acc0),
            [acc1] "+f"(acc1),
            [acc2] "+f"(acc2),
            [acc3] "+f"(acc3)
...
static inline void gemv(uint32_t trans, uint32_t m, uint32_t n, double alpha,
    ...
    if (core_m > 0)
        single_core_gemv_unrolled(trans, core_m, n, alpha, core_a, lda, x, incx,
                         &y[start_m]);
}
