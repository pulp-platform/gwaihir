...
module axi_to_detailed_mem_user #(
  ...
  typedef logic [SizeWidth-1:0] axi_size_t;

  // Local helpers replacing axi_pkg_ext::aligned_addr / axi_pkg_ext::num_bytes
  function automatic logic [127:0] aligned_addr_local(logic [127:0] addr, axi_size_t size);
    return (addr >> size) << size;
  endfunction
  function automatic int unsigned num_bytes_local(axi_size_t size);
    return 1 << size;
  endfunction
  ...
  always_comb begin
    ...
    if (w_cnt_q > '0) begin
      ...
      wr_meta        = wr_meta_d;
      wr_meta.addr   = wr_meta_q.addr + num_bytes_local(wr_meta_q.size);
      ...
      wr_meta_d = '{
          addr: addr_t'(aligned_addr_local(axi_req_i.aw.addr, axi_req_i.aw.size)),
